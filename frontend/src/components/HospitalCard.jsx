...
import { Star, MapPin, PhoneCall, Bed, HeartPulse, Building2, ShieldCheck, ArrowRight } from 'lucide-react';
    ...
    <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-md hover:shadow-xl transition-all duration-300 overflow-hidden flex flex-col justify-between group hover:-translate-y-1">
      ...
      <div>
        ...
        <div className="relative h-48 w-full overflow-hidden bg-slate-100 dark:bg-slate-800">
          ...
          />
          <div className="absolute top-3 left-3 bg-white/90 dark:bg-slate-900/90 backdrop-blur-md px-3 py-1 rounded-full text-xs font-bold text-slate-800 dark:text-slate-200 flex items-center space-x-1 border border-slate-200 dark:border-slate-700">
        ...
        <div className="p-6 space-y-3">
          <span className="text-[11px] font-bold text-sky-600 dark:text-sky-400 uppercase tracking-wider bg-sky-50 dark:bg-sky-950/60 px-2.5 py-1 rounded-full border border-sky-200 dark:border-sky-800/60">
            {hospital.type}
          </span>
          ...
          <div className="grid grid-cols-2 gap-2 pt-2 text-xs">
            <div className="bg-slate-50 dark:bg-slate-800/60 p-2.5 rounded-xl border border-slate-200/60 dark:border-slate-700/60 flex items-center space-x-2">
      ...
      <div className="px-6 py-4 bg-slate-50 dark:bg-slate-800/50 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between">
        <span className="text-xs text-slate-500">Est. {hospital.establishedYear}</span>
        <Link
          href={`/hospitals/${hospital._id}`}
          className="inline-flex items-center space-x-1.5 text-xs font-bold text-sky-600 dark:text-sky-400 hover:underline"
        >
